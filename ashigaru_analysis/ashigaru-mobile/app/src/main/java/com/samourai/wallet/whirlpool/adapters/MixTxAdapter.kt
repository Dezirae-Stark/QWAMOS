...
class MixTxAdapter(private val mContext: Context) :
    ...
    override fun onBindViewHolder(holder: TxViewHolder, position: Int) {
        ...
        if (tx!!.section == null) {
            ...
            if (tx.amount < 0.0) {
                ...
                val amountStr = "-${if (formatBalance == 1) FormatsUtil.formatSats(_amount) 
                else if (formatBalance == 2) "*********" 
                else FormatsUtil.formatBTC(
                    _amount
                )}"
                ...
                val amount =
                    if (formatBalance == 1) FormatsUtil.formatSats(_amount)
                    else if (formatBalance == 2) "*********"
                    else FormatsUtil.formatBTC(
                        _amount
                    )
    ...
    fun formatBalance(it: Int?) {
        it?.let {
            this.formatBalance = it
