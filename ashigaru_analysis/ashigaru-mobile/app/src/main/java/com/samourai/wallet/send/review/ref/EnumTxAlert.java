...
import static com.samourai.wallet.util.func.UTXOHelper.hasPath;
...
public enum EnumTxAlert {
    ...
    CONSOLIDATION {
        ...
        public TxAlertReview createAlert(final ReviewTxModel reviewTxModel) {
            return TxAlertReview.create(
                    R.string.tx_alert_title_consolidation,
                    R.string.tx_alert_desc_consolidation,
                    null);
        }
        ...
        public TxAlertReview checkForAlert(final ReviewTxModel reviewTxModel, boolean forInit) {
            ...
            final TxData txData = reviewTxModel.getTxData().getValue();
            final List<UTXO> selectedUTXO = txData.getSelectedUTXO();
            if (selectedUTXO.size() == 1) {
                return null;
            }
            if (txData.getChange() > 0L) {
                return null;
            }
            if (selectedUTXO.size() >= toUtxos(toTxOutPoints(reviewTxModel.allSpendableUtxos(false))).size()) {
                return null;
            }
            if (reviewTxModel.isPostmixAccount() && !hasPath(txData.getSelectedUTXO(),"M/1")) {
                return null;
            }
    ...
    WALLET_SWEEP {
        ...
        public TxAlertReview createAlert(final ReviewTxModel reviewTxModel) {
            return TxAlertReview.create(
                    R.string.tx_alert_title_wallet_sweep,
                    R.string.tx_alert_desc_wallet_sweep,
                    null);
        }
