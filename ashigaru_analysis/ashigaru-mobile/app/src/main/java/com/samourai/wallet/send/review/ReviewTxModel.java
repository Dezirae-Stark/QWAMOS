...
public class ReviewTxModel extends AndroidViewModel {
    ...
    public ReviewTxModel updateUtxoFilterModel(final UTXOFilterModel model) {
        ...
        if (isNull(allSpendableUtxos)) {
            allSpendableUtxos = new MutableLiveData<>(allSpendableUtxoPoints());
        } else {
            allSpendableUtxos.postValue(allSpendableUtxoPoints());
        }

        return this;
    }

    public List<UTXO> allSpendableUtxos(final boolean filter) {

        return isPostmixAccount()
                ? APIFactory.getInstance(getApplication()).getUtxosPostMix(filter)
                : APIFactory.getInstance(getApplication()).getUtxos(filter);
    }

    public List<MyTransactionOutPoint> allSpendableUtxoPoints() {

        return toTxOutPoints(applyUtxoFilter(
                allSpendableUtxos(true),
                utxoFilterModel.getValue(),
                isPostmixAccount()));
    }

    public LiveData<List<MyTransactionOutPoint>> getAllSpendableUtxosPoint() {
        if (isNull(allSpendableUtxos)) {
            allSpendableUtxos = new MutableLiveData<>(allSpendableUtxoPoints());
        }
    ...
    public void autoLoadCustomSelectionUtxos(final long minCoinValue) {
        ...
        final List<MyTransactionOutPoint> orderedOutPoints = Ordering
                ...
                .sortedCopy(allSpendableUtxoPoints());
    ...
    public ReviewTxModel buildSpendBatchTxData() throws Exception {
        ...
        final List<UTXO> utxos = getAllSelectableUtxo();
    ...
    private List<UTXO> getAllSelectableUtxo() {
