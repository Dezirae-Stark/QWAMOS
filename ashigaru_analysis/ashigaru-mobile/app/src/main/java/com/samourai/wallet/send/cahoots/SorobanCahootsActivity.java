...
public class SorobanCahootsActivity extends SamouraiActivity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        getWindow().setStatusBarColor(getResources().getColor(R.color.balance_blue));
        getWindow().setNavigationBarColor(ContextCompat.getColor(this, R.color.window));

        final int account = getIntent().getIntExtra("_account", 0);

        if (account == SamouraiAccountIndex.POSTMIX) {
            getWindow().setStatusBarColor(getResources().getColor(R.color.postmix_balance));
            final AppBarLayout viewById = findViewById(R.id.appBarLayout2);
            viewById.setBackgroundTintList(ContextCompat.getColorStateList(this, R.color.postmix_balance));
            final Toolbar toolbar = findViewById(R.id.toolbar);
            toolbar.setBackgroundColor(getResources().getColor(R.color.postmix_balance));
            final CahootsCircleProgress stepView = findViewById(R.id.step_view);
            stepView.setBackgroundColor(getResources().getColor(R.color.postmix_balance));
            final ViewPager viewFlipper = findViewById(R.id.view_flipper);
            viewFlipper.setBackgroundColor(getResources().getColor(R.color.windowDark));
        }
