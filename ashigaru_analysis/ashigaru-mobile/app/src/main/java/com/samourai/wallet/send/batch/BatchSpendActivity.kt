            ...
            if (PrefsUtil.getInstance(applicationContext).getValue(PrefsUtil.RBF_OPT_IN, false)) {
                ...
                for (input in tx!!.inputs) {
                    ...
                    if (path != null) {
                        ...
                        when {
                            _isBIP84 -> {
                                rbf.addKey(input.outpoint.toString(), "PCODE/$pcode/$idx/84")
                            }
                            _isBIP49 -> {
                                rbf.addKey(input.outpoint.toString(), "PCODE/$pcode/$idx/49")
                            }
                            else -> {
                                rbf.addKey(input.outpoint.toString(), "PCODE/$pcode/$idx")
                            }
                        }
                    }
                }
