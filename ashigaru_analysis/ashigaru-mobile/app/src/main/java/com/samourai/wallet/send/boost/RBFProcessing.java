...
public class RBFProcessing {
    ...
    private void loadKeyBags(final Map<String, ECKey> keyBagLegacy,
        ...
        for (final Map.Entry<String, String> keyByOutpoint : keys.entrySet()) {
            ...
            if (StringUtils.startsWith(key, "PCODE/")) {
                keyPath = Arrays.copyOfRange(StringUtils.split(key, "/"), 1, 3);
            } else {
                keyPath = StringUtils.split(key, "/");
            }
            Log.i("RBF", "path length:" + keyPath.length);
            final ECKey ecKey = getECKey(keyPath, activity.getAccount(), outpoint);
            ...
            if (nonNull(ecKey)) {
                if (extraInputs.containsKey(outpoint)) {
                    ...
                    if (keyPath.length == 4) {
                        if (keyPath[3].equals("84")) {
                            keyBag84.put(outpoint, ecKey);
                        } else {
                            keyBag49.put(outpoint, ecKey);
                        }
                    } else {
                        final String[] fullPath = StringUtils.split(key, "/");
                        if (fullPath.length == 4) {
                            if (fullPath[3].equals("84")) {
                                keyBag84.put(outpoint, ecKey);
                            } else {
                                keyBag49.put(outpoint, ecKey);
                            }
