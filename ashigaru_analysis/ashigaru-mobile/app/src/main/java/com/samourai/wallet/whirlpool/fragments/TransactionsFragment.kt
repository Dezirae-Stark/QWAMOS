...
class TransactionsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        whirlPoolHomeViewModel.onboardStatus.observe(viewLifecycleOwner, { showOnBoarding ->
            if (showOnBoarding) {
                ...
                val balanceFormat = PrefsUtil.getInstance(requireActivity()).getValue(PrefsUtil.BALANCE_FORMAT, 0)
                adapter?.formatBalance = balanceFormat
    ...
    override fun onCreateView(
        ...
        whirlPoolHomeViewModel.formatBalanceLive.observe(viewLifecycleOwner, { format->
            adapter?.formatBalance(format)
        })
