...
public class RBFFactory {
    ...
    public static RBFSpend createRBFSpendFromTx(final Transaction tx, final Context context) {
        ...
        for (final TransactionInput input : tx.getInputs()) {
            ...
            final String path = APIFactory.getInstance(context).getUnspentPaths().get(_addr);
            if (path != null) {
                if (_isBIP84) {
                ...
                final String pcode = BIP47Meta.getInstance().getPCode4Addr(_addr);
                final int idx = BIP47Meta.getInstance().getIdx4Addr(_addr);

                if (_isBIP84) {
                    rbf.addKey(input.getOutpoint().toString(), "PCODE/" + pcode + "/" + idx + "/84");
                } else if (_isBIP49) {
                    rbf.addKey(input.getOutpoint().toString(), "PCODE/" + pcode + "/" + idx + "/49");
                } else {
                    rbf.addKey(input.getOutpoint().toString(), "PCODE/" + pcode + "/" + idx);
                }
            }
