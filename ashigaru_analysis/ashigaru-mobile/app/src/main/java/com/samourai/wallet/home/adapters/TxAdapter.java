...
public class TxAdapter extends RecyclerView.Adapter<TxAdapter.TxViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TxViewHolder holder, int position) {
        Integer balanceFormat = PrefsUtil.getInstance(this.mContext).getValue(PrefsUtil.BALANCE_FORMAT, 0);
        ...
        if (tx.section == null) {
            ...
            if (tx.getAmount() < 0.0) {
                ...
                String concatStr = "";
                if (balanceFormat == 1)
                    concatStr = FormatsUtil.formatSats(_amount);
                else if (balanceFormat == 2)
                    concatStr = "*********";
                else
                    concatStr = FormatsUtil.formatBTC(_amount);
                holder.tvAmount.setText("-".concat(concatStr));
                ...
                String amount = "";
                if (balanceFormat == 1)
                    amount = FormatsUtil.formatSats(_amount);
                else if (balanceFormat == 2)
                    amount = "*********";
                else
                    amount = FormatsUtil.formatBTC(_amount);
