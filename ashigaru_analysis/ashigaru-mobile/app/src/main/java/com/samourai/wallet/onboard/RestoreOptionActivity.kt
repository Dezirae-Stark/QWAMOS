...
class RestoreOptionActivity : AppCompatActivity() {
    ...
    private fun restoreWalletFromBackup() {
        ...
        fun initializeRestore(decrypted: String, skipDojo: Boolean) {
            ...
            scope.launch(Dispatchers.IO) {
                ...
                PrefsUtil.getInstance(applicationContext).setValue(PrefsUtil.WALLET_SCAN_COMPLETE, false)
                ...
                if (it != null) {
                    ...
                    val dojoCredsMap: HashMap<String, String>? =
                        getDojoCredentialsFromBackup(decrypted)

                    val intent = if (AppUtil.getInstance(applicationContext).isOfflineMode) Intent(
                        applicationContext,
                        OfflineDojoActivityScreen::class.java
                    )
                    else Intent(applicationContext, SetDojoActivity::class.java)

                    if (dojoCredsMap != null) {
                        intent.putExtra("dojoURL", dojoCredsMap["dojoURL"])
                        intent.putExtra("explorerURL", dojoCredsMap["explorerURL"])
                        intent.putExtra("apikey", dojoCredsMap["apikey"])
                    }
    ...
    fun getDojoCredentialsFromBackup(decryptedString: String): java.util.HashMap<String, String>? {
        val dojoCredsMap = java.util.HashMap<String, String>()
        try {
            val decryptedJson = JSONObject(decryptedString)
            val dojoJson = decryptedJson.getJSONObject("meta").getJSONObject("dojo")
                .getJSONObject("pairing")
            val explorerURL = if (decryptedJson.getJSONObject("meta").has("explorer_url")
            ) decryptedJson.getJSONObject("meta").getString("explorer_url")
            else ""

            dojoCredsMap["dojoURL"] = dojoJson.getString("url")
            dojoCredsMap["apikey"] = dojoJson.getString("apikey")
            dojoCredsMap["explorerURL"] = explorerURL
