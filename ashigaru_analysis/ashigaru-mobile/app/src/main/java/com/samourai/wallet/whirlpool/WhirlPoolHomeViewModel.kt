...
class WhirlPoolHomeViewModel : ViewModel() {
    ...
    fun switchBalanceFormat(int: Int?=null, context: Context): Int {
        if(int!=null){
            this.formatBalance.value = int
            return formatBalance.value!!
        }else{
            val balanceFormat =
                PrefsUtil.getInstance(context).getValue(PrefsUtil.BALANCE_FORMAT, 0)

            when (balanceFormat) {
                0 -> {
                    formatBalance.value = 1
                    return formatBalance.getValue()!!
                }

                1 -> {
                    formatBalance.value = 2
                    return formatBalance.getValue()!!
                }
